...
exports.signup = async (req, res, next) => {
  ...
  try {
    ...
    const token = new Token({ _userId: result._id, token: crypto.randomBytes(16).toString('hex') });
    const tokenResult = await token.save();

    sendEmail(email, templates.confirm(tokenResult.token, firstName));
    res.status(201).json({ message: 'User created!', userId: result._id, email });
  } catch (err) {
    if (!err.statusCode) {
      err.statusCode = 500;
    }
